...
def make_csv(newval,cnt,subject_code):
    ...
    with open("result_"+filename[cnt]+".csv", mode='w',newline='') as csv_file:
    ...
    with open("result_"+filename[cnt]+".csv", 'r') as f, open("result_RET_"+filename[cnt]+".csv", 'w') as fo:
        for line in f:
            fo.write(line.replace('"', '').replace("'", ""))
    os.remove("result_"+filename[cnt]+".csv")
